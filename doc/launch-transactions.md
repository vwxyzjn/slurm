...
This change targets the upstream Slurm 25.05.3 release (`slurm-25-05-3-1`).
It coordinates preemption and resource allocation for ordinary jobs,
heterogeneous jobs, and job arrays. The source changes apply directly to that
release and do not require a container-vendor patch stack.
...
This release-based change has not been ported to later Slurm releases or the
current upstream development branch.
